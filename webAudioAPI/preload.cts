...
let workletLoading: Promise<boolean> | undefined;
...
(async () => {
    ...
    function cleanUp(id: string) {
        if (!player[id]) { console.log("[preload] cleanUp: already cleaned", id); return; }
    ...
    async function createSource(fileId: string, playerId: string, audioBuffer: AudioBuffer) {
        const source = context.createBufferSource();
        source.buffer = audioBuffer;
        const gain = context.createGain();

        let rubber: AudioWorkletNode | null = null;
        ...
        if (ok) {
            ...
            rubber.port.onmessage = (msg) => {
                ...
                ipcRenderer.send("worklet-log-" + playerId + "-" + fileId, msg.data);
            };
    ...
    function removeSource(fileId: string, playerId: string) {
        ...
        try { player[playerId].sources[sourceIndexNum].rubber?.disconnect(); } catch { }
