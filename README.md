...
# ブランチ運用方針
このリポジトリではv2.0をmainブランチにマージしたタイミングから次のタイミングで運用しています(します)。

- main: １月、４月、７月、１０月にbetaからマージ
- beta: 他ブランチで開発した機能が安定したらマージ
- alpha: 開発したコードをとりあえず動作させたい時にマージ(挙動不安定時にbetaからマージして上書き)
...
## mainブランチの使い方
mainブランチはほとんど変更しません。ただ、`README.md`の修正などと言ったものはmainブランチに直接コミットすることがありますが、基本的にコミットは３ヶ月に１度とします。

ただし、重大なバグ修正・機能追加要望に限り、わかりやすいコードかつ検証ができた場合は直接マージしても問題はありません。

さらに、ここはバージョンに関わります。v1.0.0などとバージョンをつけます。vメジャー.マイナー.パッチという命名規則を利用しますが、これらのタイミングを説明します。
- メジャー: ３ヶ月に１回のbetaブランチからのマージ時
- マイナー: 機能追加要望で期待値が高い機能を、alpha、betaともにバグがない状態だと判断した時
- パッチ: 実際に運用している間に起きるエラーを修正せざるを得ない場合にbetaで検証が完了した時
...
例えば１１月１日になったらbetaブランチには新しい機能のマージをしてはなりませんが、mainブランチにbetaブランチがマージされたら、すぐに利用することができます。１月の１日や５日などバラバラにはなりますが、すぐに利用することが可能になるはずです。細かいタイミングは実際にmainブランチがマージされたかどうかを確認してください。

betaに実験段階のコードを入れるのを避けましょう。betaブランチは未熟なブランチとはいえ、利用される前提にあります。無駄な再起動は控えめに。

## alphaブランチの使い方
alphaブランチはとりあえず作った機能をマージしても良い環境です。まだbetaにマージしてない機能だけど組み合わせて使ってみたい機能や、テストしてみたいものがあった時に使います。

alphaブランチは自由にマージして構いません。しかし、それがbotに反映されるとも限りません。実際に反映するのはbotを展開している本人であるazkazunami36次第です。

alphaブランチはbetaからマージすることで状態を修正することが多々あります。前に適用した機能が削除されているなんて当たり前です。

alphaブランチに使用されるbotの再起動頻度はとても高く、バグも多かったり、セキュリティ面で問題が大量にあります。開発者向けであり、興味があるだけでは利用してはならず、覚悟も必要です。

## 開発用ブランチの使い方
このリポジトリでは開発用ブランチを使って開発することを推奨します。

- new/名前: 新機能を実装する時に使うブランチです。betaから派生してください。
- fix/名前: バグ修正を実装する時に使うブランチです。betaのバグを修正する時に使ってください。
- refactor/名前: リポジトリ内のファイル構造の変更、または関数・クラス・機能の位置の変更時に使うブランチです。betaから派生してください。あまり作らないでください(詳細は後述)

上記の通りにブランチを作るとみやすくなるかと思います。

### 開発用ブランチをbetaにマージする際の注意
開発時にブランチを作るのは構いませんが、betaにマージする際にもし`refactor`ブランチが存在する場合、あなたの開発しているコードの位置が変更されている可能性があります。その場合、`refactor`ブランチがマージされるのを待ってください。
