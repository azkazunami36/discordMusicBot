...
export class EditMusicAddFilePopup extends EventEmitter {
    ...
    editMusicInfoAddFileTabManager;
    mainElement;
    constructor(popupManager) {
        super();
        this.popupManager = popupManager;
        this.editMusicInfoAddFilePopup = document.getElementById("editMusicInfoAddFilePopup");
        this.mainElement = this.editMusicInfoAddFilePopup.getElementsByClassName("main")[0];
    ...
    addAndClose() {
        ...
        if (fileName) {
            // 編集データにサウンド情報を追加
            if (!this.popupManager.popupClassList.editMusicPopup.editingData)
                this.popupManager.popupClassList.editMusicPopup.editingData = {};
            const editingData = this.popupManager.popupClassList.editMusicPopup.editingData;
            if (!editingData.sounds)
                editingData.sounds = [];
            if (!editingData.sounds[0])
                editingData.sounds[0] = { languagetype: "ja" };
            if (!editingData.sounds[0].filelist)
                editingData.sounds[0].filelist = [];
            if (!editingData.sounds[0].filelist.find(value => value.filename === fileName))
                editingData.sounds[0].filelist.push({
