...
export class ListManagerV2 extends EventEmitter {
    firstElement;
    mainElement;
    tableElement;
    tHeadElement;
...
;
export class ListManager extends EventEmitter {
    firstElement;
    mainElement;
    tableElement;
    tHeadElement;
            ...
            firstElement.addEventListener("mousedown", e => {
                ...
                if (listButton?.classList.contains("listButton")) {
                    ...
                    if ((Date.now() - mousedownTime) < 250) {
                        ...
                        let className;
                        if (activedElement)
                            activedElement.classList.forEach(value => { if (value.includes("selectingName"))
                                className = value; });
                        if (className)
                            className = decodeURIComponent(atob(className.split("-")[1]));
